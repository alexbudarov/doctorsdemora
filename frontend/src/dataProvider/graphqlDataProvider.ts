...
import { filterFilesFromData } from "./filterFilesFromData";
...
export const dataProvider: DataProvider = {
  ...
  update: async (resource, { id, data, meta }): Promise<UpdateResult> => {
    // filter private sources for working with files before sending it to backend
    const dataWithoutFiles = filterFilesFromData(data);
    ...
    const updateData: { id: Identifier; __typename?: string } = filterPropsWithNullId(
      stripTypename({ id, ...dataWithoutFiles })
    );
...
export const getSelectionSetName = (query: Record<string, any>): string => {
