...
export const authProvider: AuthProvider = {
  ...
  checkAuth: async (): Promise<void> => {
    ...
    await apolloClient.query({ query: USER_AUTHENTICATED }).catch((e) => {
      if (e.name === "ApolloError" && e.message === "Failed to fetch") {
        // do not perform logout in case of request cancel
        // in future we can add special error in apollo client for cancel requests
        // and handle it here more precise
        // fix for https://github.com/Amplicode/amplicode-frontend/issues/1149
        console.log(
          "checkAuth: looks like request for auth check canceled, do not perform checkout"
        );
      } else {
        throw e;
      }
